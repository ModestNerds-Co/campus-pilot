...
pub fn module_catalog() -> Vec<ModuleDefinition> {
    vec![
        ...
        module(
            ...
            "Transport",
            "Campus operations",
            "Configure routes and rider assignments; operate dated manifests.",
            "/modules/transport",
            "transport",
            false,
            "available",
            &["view", "configure", "operate", "manage"],
        ),
        module(
            "hostel",
            "Hostel",
    ...
    async fn module_coverage_exposes_current_release_and_agent_gaps() {
        ...
        for module_key in [
            ...
            if module_key == "administration" {
                ...
                assert!(module.release_ready());
                assert_eq!(module.executable_capabilities(), 31);
                ...
                assert!(module.release_ready());
                assert_eq!(module.routed_operations(), 66);
                assert_eq!(module.exposed_operations(), 21);
                assert_eq!(module.approval_required_operations(), 39);
                assert_eq!(module.human_only_operations(), 6);
                assert_eq!(module.executable_capabilities(), 21);
