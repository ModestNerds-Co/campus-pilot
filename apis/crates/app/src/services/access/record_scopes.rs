...
pub const RECORD_SCOPE_FAMILIES: &[RecordScopeFamilyDefinition] = &[
    ...
    definition("student_support.cases", CAMPUS_ASSIGNED),
    definition("activities.groups", CAMPUS_SELF_ASSIGNED),
    definition("activities.sessions", CAMPUS_SELF_ASSIGNED),
    ...
    definition("health.patients", CAMPUS_SELF),
    definition("health.care", CAMPUS_SELF),
...
mod tests {
    ...
    fn catalogue_keys_are_unique_and_parse_safe() {
        assert_eq!(RECORD_SCOPE_FAMILIES.len(), 43);
    ...
    fn family_catalogue_rejects_unsupported_scope_kinds() {
        ...
        assert!(RoleRecordScopeAssignment::parse("health.patients", "campus").is_ok());
        assert!(RoleRecordScopeAssignment::parse("health.patients", "assigned").is_err());
        assert!(RoleRecordScopeAssignment::parse("health.care", "assigned").is_err());
