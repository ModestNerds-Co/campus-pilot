...
- Academics owns academic years, non-overlapping academic terms, canonical tenant-wide grade levels, subjects, classes, employee-backed teacher profiles, and teaching assignments. Classes reference a canonical grade-level identifier rather than storing a copied label. CSV/XLSX imports must resolve external grade codes or labels through their saved mapping before preview commit; an unresolved grade blocks that row. Terms remain inside their academic year and move forward through planned, active, and closed states. Timetabling requires the active term, consumes Academics records through typed `cp-academics` operations, and consumes only the minimum approved HR availability fields needed for that term. Dated HR availability is a planning constraint, never a permanent weekly slot. Timetabling owns recurring schedule rules, rooms, room preferences, immutable generated snapshots, and publication state. SIS remains the source of learner enrolment and class placement.
