...
export const BootScreen: React.FC<BootScreenProps> = ({ className = '' }) => {
  const [status, setStatus] = useState<'loading' | 'offline' | 'error' | 'success'>('loading');
  ...
  const checkBootstrapStatus = async (isRetry = false) => {
    ...
    try {
      ...
      if (err instanceof Error && (err.message.includes('network') || err.message.includes('Network'))) {
        setStatus('offline');
      } else {
          ...
          <div className="text-center space-y-6">
            ...
            <div className="space-y-4">
              ...
              <p className="text-gray-600 max-w-md mx-auto leading-relaxed">
                Setup can proceed offline. Changes will sync when internet becomes available.
              </p>
              <div className="pt-2">
                <button
                  onClick={handleRetry}
