...
import { ArrowLeft, ArrowRight, User, Eye, EyeOff, Loader2, AlertCircle, Shield, Check } from 'lucide-react';
import { bootstrapService } from '../../services/bootstrapService';
import type { AdminFormData, FormFieldError, PasswordStrength } from '../../types';
import { validateEmail, validatePhone, validatePassword, checkCapsLock } from '../../../../lib/validation';
import { PASSWORD_STRENGTH_COLORS, PASSWORD_STRENGTH_LABELS } from '../../constants';
...
export const AdminSetupScreen: React.FC = () => {
  ...
  const validateForm = (): boolean => {
    ...
    } else if (formData.full_name.trim().length < 2 || formData.full_name.trim().length > 80) {
      newErrors.push({ field: 'full_name', message: 'Full name must be between 2 and 80 characters' });
    }
    ...
    if (!passwordStrength.isValid) {
      newErrors.push({ field: 'password', message: 'Password does not meet security requirements' });
    }

    // Confirm password
    if (formData.password !== formData.password_confirm) {
      newErrors.push({ field: 'password_confirm', message: 'Passwords do not match' });
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        toast.success(response.message || 'Administrator account created successfully');
        ...
        toast.error(response.message || 'Failed to create administrator account');
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-gray-50">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-4">Create the first administrator</h1>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <p className="mt-2 text-sm text-gray-500">At least 10 characters, including a number and symbol.</p>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className={`w-full px-4 py-3 pr-12 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors ${
                    getFieldError('password_confirm') ? 'border-red-500' : 'border-gray-300'
                  }`}
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-blue-700">
                This administrator account will have full access to the system. Keep your credentials secure
                and use a strong, unique password. You can create additional admin accounts later.
              </p>
            </div>
          </div>
        </div>
