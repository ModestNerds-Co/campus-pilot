...
The pure operation evaluator returns stable reason codes for module, feature, local enablement, dependency, lease lifecycle, application version, permission, record scope, quota, and approval decisions. Operation catalog version 1 assigns stable, exact descriptors to the 43 currently implemented authenticated campus operations across licensing, campus settings, roles, users, fleet, vehicle logs, and timetabling. Actix route-pattern tests verify that every descriptor resolves to its intended route, and catalog tests reject duplicate keys, duplicate route identities, unknown modules, and unknown permissions. Permission-gated routes now use the descriptor's exact module and permission instead of inferring them from a URL namespace and HTTP method; this corrects Vehicle Logs ownership under Fleet and licensing repair actions under `licensing:edit`. The full evaluator still runs in shadow mode, while the descriptor-backed compatibility module/permission check or existing authentication-only behavior remains authoritative until lifecycle drift is reviewed. Agent discovery hides unlicensed capabilities, but every Agent execution evaluates licensing, permissions, scope, policy, approval, and quota again.
