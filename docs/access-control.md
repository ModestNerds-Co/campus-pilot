...
- Assets and inventory owns the tenant item master and store directory. The server assigns immutable, monotonic `ITM-000001` and `STR-000001` references; clients never supply or recycle them. Item quantities use an exact scaled-integer contract: `quantity_scale` is fixed when the item is created, while thresholds such as `reorder_level_minor` use that scale and never use floating-point values. Item and store CRUD requires only the Assets and inventory entitlement. Future Procurement receipt allocation, Finance valuation, and HR-owned employee custodianship will declare those owning modules as dependencies when those operations exist. This foundation does not yet create a stock ledger, stock balances or movements, custodianship, Finance postings, or allocations from Procurement goods receipts; item and store CRUD cannot imply any of those effects.
