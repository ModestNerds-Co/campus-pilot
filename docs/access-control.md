...
The pure operation evaluator returns stable reason codes for module, feature, local enablement, dependency, lease lifecycle, application version, permission, record scope, quota, and approval decisions. Operation catalog version 2 assigns stable, exact descriptors to the 282 currently implemented authenticated campus operations across licensing, campus settings, roles, users, AI-provider and routing Administration, HR and payroll, Fleet, vehicle logs, Academics, Timetabling, SIS, Finance, Fees, Procurement, and Assets and inventory. Each descriptor also has a mandatory Agent exposure classification; the current catalog records 115 exposed reads, 157 approval-required mutations, and ten human-only operations with explicit reasons. AI-provider Administration requires the Agent entitlement and exact `ai_providers:view` or `ai_providers:edit` authority; credential creation, rotation, and destructive disconnect stay direct human workflows. Agent routing likewise requires the Agent entitlement plus exact `ai_routing:view` or `ai_routing:edit` authority. Route list, read, fail-closed resolution, and the secret-free route-options projection are exposed reads; create, replace, and archive require approval before Agent can execute them. The options projection makes route administration independent from provider-administration permission without exposing credentials, fingerprints, actor metadata, or internal model-snapshot IDs. All 27 Procurement operations require its own entitlement plus HR and payroll and Finance, so requesters and currencies remain owned by their canonical modules. Purchase-order and goods-receipt list/read operations are exposed to Agent through reduced sensitive projections. Purchase-order create, update, issue, and cancel plus goods-receipt create, update, and post are the seven new approval-required mutations, alongside the ten existing supplier and requisition mutations. Assets and inventory has 20 routed operations: eight exposed reads and 12 approval-required writes. The four stock-ledger reads are balance list, movement list/read, and goods-receipt-allocation source list; the six stock-ledger writes are manual receipt, issue, transfer, adjustment, reversal, and goods-receipt allocation. Only the `assets_inventory.goods_receipt_allocations.list` and `assets_inventory.goods_receipt_allocations.create` operations require Procurement; the other 18 Assets operations, including reversal, require no cross-module entitlement, and none requires Finance or HR and payroll. This is coverage policy rather than execution authority: only a broker-backed typed capability may execute an exposed or approval-required operation. Actix route-pattern tests verify that every descriptor resolves to its intended route, and catalog tests reject duplicate keys, duplicate route identities, unknown modules, unknown permissions, and missing Agent classifications. Catalogued permission-authoritative routes enforce the complete evaluator decision at the API boundary instead of inferring access from a URL namespace and HTTP method. This includes lease lifecycle and module state, keeps cross-module dependency checks code-owned, keeps licensing repair under `licensing:edit`, and returns a generic denial without exposing commercial or authorization internals. Only launcher catalog and tenant-module discovery are authentication-only because every signed-in role needs them before entering a module. Licensing status and school settings are permission-authoritative. Agent discovery hides unlicensed capabilities, but every Agent execution evaluates licensing, permissions, scope, policy, approval, and quota again.
...
- Academics is authoritative for the active academic year and term, canonical grade levels, classes, subjects, employee-backed teachers, teaching assignments, and assessment structures. SIS applications and staged imports resolve grade references through Academics; Timetabling rehydrates academic records through typed Academics operations and never accepts client copies as authoritative. Assessment components resolve class, subject, and teacher through their teaching assignment. Learner marks later consume SIS enrolments through a typed owning-module boundary.
