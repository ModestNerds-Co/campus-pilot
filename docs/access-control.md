...
- SIS data imports are licensed SIS operations. CSV/XLSX source bytes and unmapped source columns are retained privately with a fingerprint, never returned by read APIs or Agent capabilities. Every mapping creates a new immutable version and normalized preview, and only an explicit idempotent commit creates ready learner or guardian rows. Invalid and duplicate rows retain results; imports never create user accounts. The shared parser is destination-neutral so HR, Finance, and later modules can reuse the staged contract while keeping their own canonical validation and commit authority.
