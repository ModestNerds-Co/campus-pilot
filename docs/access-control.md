...
- Timetabling consumes active Academics records and owns only scheduling rules, rooms, teacher unavailability, generated snapshots, and publication state. Timetabling entitlements include Academics and its transitive HR and payroll dependency.
...
- Timetabling owns teaching days, periods, rooms, teacher unavailability, per-assignment room preferences, generated snapshots, and publication state. A teacher name change in HR is reflected in new configuration reads without rewriting an existing timetable snapshot.
