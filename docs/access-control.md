...
- A learner is the canonical person/enrolment record in People and admissions. Ordinary learner creation transactionally assigns an immutable number from the tenant-owned prefix, padding, and monotonic sequence policy (default `LRN-000001`); staged imports preserve explicit legacy learner numbers and align values in the managed namespace before commit. Student login is an optional linked account. Guardian relationships attach to the learner/person records and may independently link to user accounts.
