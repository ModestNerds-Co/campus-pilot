...
- Fees owns one immutable-identity billing account per SIS learner and versioned fee structures scoped to Academics-owned years, terms, and grades. Fee values use the selected Finance currency's integer minor units; structures reference active Finance posting asset and income accounts through typed owning-module operations. Draft structures may be edited or removed, active structures may only be retired, and closed billing accounts cannot be reopened. Billing-account reads are campus-wide only for Fees administrators; other `fees:view` users are restricted to learner records linked to their current login. Fees operations declare SIS, Academics, and Finance as licensed dependencies. Later invoices, receipts, adjustments, and imported billing records remain Fees source records and send typed balanced posting requests to Finance instead of writing ledger rows.
