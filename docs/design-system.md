...
- E-learning uses full-page space, unit, assignment, learner-work, review, and Gradebook score-transfer workspaces. Preparing and reviewing a cross-module score transfer stays on a full page; only the short final apply or reject decision uses a right-side drawer. Space, unit, resource, publication, withdrawal, archive, settings, assignment authoring, file removal, submission confirmation, and feedback-release workflows use the shared right-side drawer where secondary focus is needed; published content is presented as read-only rather than with disabled editing forms. Learner file upload remains inline in the learner-work page so a drawer never contains another scrolling file workflow.
...
- Multi-stage CSV/XLSX imports keep upload, column mapping, row validation, and preview tables on a full page. Only the short final commit confirmation uses a right-side drawer.
