...
Current implementation status: all 649 released server-owned operations have a code-owned Agent exposure classification. Two hundred and forty-one read operations are exposed, 366 mutations require approval, 30 credential, destructive connection, private-document-byte, restricted-reference, configuration-reference, role, account-deletion, license-credential, and Facilities and Activities assignment-reference operations remain human-only with explicit reasons, and 12 operations are prohibited. This metadata is coverage policy only; it does not make an operation executable until the capability broker and typed capability handler exist.
...
Current implementation status: `cp-agent` provides the typed, provider-independent broker boundary. It owns versioned capability descriptors and closed input/output schemas, a registry indexed against the complete product-operation catalogue, authenticated principal and proof-bearing scope types, typed handler adapters, fresh authority and record-scope checks, reserved identity-field rejection, and a fail-closed `cp-audit` sink. The application assembles directly exposed reads over the same typed domain operations as the normal API. Health services exposes patient references, scoped patient records, visits, medication plans, immutable administration history, and follow-ups through eight typed read handlers. All Health capability results are Highly Sensitive and provider data is `local_only`; an external AI provider is never selected for them. The 11 Health mutations remain approval-required and unregistered. Library exposes settings, borrower references, catalogue titles and copies, memberships, loans, holds, and fines through fourteen typed read handlers. Membership, loan, hold, and fine handlers reload current campus or self scope; exact record reads prove tenant ownership, and fine results are Sensitive. Its 20 write operations remain approval-required and unregistered. SIS includes a tenant-wide, General-sensitivity learner-numbering policy handler that returns only prefix, padding, next sequence, rendered preview, exhaustion status, and optimistic version; the approval-required policy mutation is not registered for execution. Procurement exposes active Finance currencies, minimum-field HR requester candidates, suppliers, requisitions, purchase orders, and goods receipts. Supplier, requisition, purchase-order, and receipt data is sensitive; those adapters remove HR login links and internal mutation-actor identifiers before results can enter model context. Purchase-order reads retain immutable requisition, requester, supplier, currency, and line snapshots plus receiving state. Receipt reads retain the Procurement-owned delivery record with its requisition, supplier, currency, and order-line snapshots; they do not expose a Finance posting because receiving creates none. Finance reads expose currency, chart-of-account structure, fiscal years, accounting periods, journal lists, journal records, journal validation, and module posting requests; journal and posting-request data is sensitive, and balances or statements are not yet exposed. Fees exposes learner candidates, billing accounts, fee structures, invoices, staged billing-account imports, and their Academics and Finance reference data. Learner candidates and import metadata are personal; billing-account, invoice, and normalized import-preview data is sensitive; and the adapter reloads current access so non-administrative reads remain restricted to learner records linked to the caller's login. HR employment and import reads are personal; availability and import-preview reads are sensitive, and the separate typed scheduling reference omits notes and decision metadata. The HTTP licensing route and Agent adapter share one safe licensing read model, so persisted installation and signed-lease interpretation cannot drift. Academics capabilities resolve teacher identity through HR-owned employee operations; term, grade-level, and assessment-cycle reads expose general academic structure, while assessment-component reads are personal because they resolve the assigned teacher. Timetabling capabilities resolve canonical academic records through Academics-owned operations, bind generated snapshots to the active term, and expose run-history reads classified as personal; and SIS capabilities resolve academic placements through Academics while keeping learner, guardian, and import-preview data explicitly classified as personal or sensitive. SIS, HR, and Fees import capabilities expose bounded metadata and normalized preview reads but never retained source bytes or unmapped source columns; upload, mapping/preview creation, and commit remain approval-required mutations. List inputs use bounded typed pagination/filter enums; record reads declare proof-bearing resource scopes; personal data is classified explicitly; and reserved authenticated identity fields cannot be supplied by model input. Catalogue, dependency, authority, and invalid-input paths are verified to fail safely. The first release deliberately registers only directly exposed read/export operations with no approval requirement; approval-required mutations, including every purchase-order or receiving state change, cannot be registered until immutable proposals and approval execution exist. The registry is held in application state. Provider connections, ordered routing, encrypted provider execution, durable Sessions/runs/usage, the internal worker, Agent workspace, session history, and global drawer are implemented. The Agent module remains `planned` and is not exposed until a licensed, provider-configured end-to-end release gate passes.

Assets and inventory exposes 13 typed reads: item and store lists/records, stock balances, stock-movement list/read, goods-receipt-allocation sources, and stock-request list/read plus requester and department references resolved from HR. The Agent handlers return reduced projections that retain generated references, exact scaled quantities, movement provenance, and the minimum allocation and request fields while omitting internal actor identifiers and unrelated Procurement or HR data. Item/store maintenance, manual stock movements, reversal, goods-receipt allocation, and stock-request submission, approval, rejection, cancellation, and fulfilment comprise 21 approval-required mutations. None is registered for direct execution until the immutable proposal and approval broker exists; a model cannot change requests or write the stock ledger through a read handler.

Transport exposes five typed reads: bounded route and rider lists, exact route records, bounded run lists, and exact run/manifest records. The handlers call Transport-owned operations that resolve current learner identity through SIS and current vehicle/employee-backed driver status through Fleet; they never return installation secrets, user credentials, internal actor identifiers, or a second people directory. Route and run identifiers are retained only where they are needed for an exact follow-up capability. The 13 Transport mutations remain approval-required and unregistered, while the learner/vehicle/driver reference picker remains human-only.

The current executable registry contains 241 reads: 16 Administration, 13 HR and payroll, seven Fleet, five Transport, six Facilities, six Activities, 31 Academics, eight Attendance, 21 Learning, three Student Support, seven Communication, four Timetabling, 15 SIS, 12 Finance, 11 Fees, 14 Library, 11 Hostel, eight Health services, 11 Document Registry, nine Internal Audit, ten Procurement, and 13 Assets and inventory. Facilities exposes scoped location, service-request, and work-order list and exact reads through typed handlers. Each handler reloads the current person, permissions, entitlement, and record-scope evidence; request reads apply campus or self scope and work-order reads apply campus or assigned-employee scope before pagination. Its 11 mutations remain approval-required and its employee/location assignment-reference workflow remains human-only. Internal Audit exposes numbering-policy, plan, engagement, evidence, auditor-candidate, and finding reads through typed handlers. Plan scope is campus-wide; engagement, evidence, and finding scope is campus-wide for Audit Managers and assigned-only for Internal Auditors. Evidence results contain the governed reference snapshot and purpose only—never Document Registry bytes, object keys, hashes, download URLs, or private actor identifiers. Its 17 mutations remain approval-required and unregistered. Hostel exposes SIS-backed learner references, residences, rooms, allocation previews and records, transfer previews, and pastoral records through scoped typed reads. Occupancy reads re-evaluate current campus or self scope; pastoral results require campus scope and are Sensitive. Its 12 mutations remain approval-required and unregistered. Health reads use typed SIS and HR dependencies, re-evaluate campus or self scope, and are restricted to local processing because all results are Highly Sensitive; its 11 mutations remain approval-required. Library reads use typed SIS, HR, Finance, and Fees dependencies; borrower records enforce campus or self scope, while every Library mutation remains approval-required. Attendance exposes assigned-class reference data, bounded register and lesson-session lists, exact register and lesson-session reads, learner history over current accepted submitted snapshots, and campus-only exception lists and records. Every handler reloads `attendance.registers` scope, teachers resolve current classes through HR-backed Academics teaching assignments, and visibility is applied before pagination. Lesson sessions additionally require Timetabling. Individual learner marks, history, and exceptions are Sensitive; exact reads carry tenant-proven register, lesson-session, exception, or learner scopes. The 11 Attendance mutations remain approval-required. Learning additionally exposes scoped quiz lists and records, learner-self or assigned-teacher attempt history, current completion policy, personal completion, and class completion. Learner projections never serialize correct-answer flags, every handler reloads `learning.spaces` scope before returning records, and the 39 Learning mutations remain approval-required and unregistered. Score-transfer list and exact-record reads expose immutable proposal evidence through typed Sensitive handlers; create, apply, and reject remain approval-required and are not registered for direct Agent execution. Learner submission uploads, draft-file removal, and signed downloads are human-only; Agent sees governed metadata only and never receives private bytes, object keys, hashes, or signed URLs. Student Support exposes bounded assigned or campus case lists, exact case records, and append-only case-action history through three typed reads. Each handler reloads the current user, access context, and `student_support.cases` scope; removal from an active case team revokes access on the next call. Results are Highly Sensitive and local-only. Reference data remains human-only, while eight mutations remain approval-required and unregistered. Communication exposes scoped audience references, announcement lists and records, recipient previews, delivery history, and the authenticated person's inbox. Each handler reloads current role-scope evidence before querying; personal inbox operations derive the account only from the authenticated Agent principal. Its eight mutations remain approval-required and publication is classified as an external side effect. Gradebook registers reduced reference, mark-sheet, and mark-import metadata and normalized-preview handlers, but those six reads are withheld from initial worker discovery until the broker passes current role-scope evidence into the handlers; raw import bytes and unmapped source columns never enter Agent context. Its HTTP routes already enforce campus or assigned-teacher visibility, and its nine mutations remain approval-required. Progress & reporting registers six typed reads for references, grading schemes, report batches, and published transcripts. Those reads are also withheld from initial discovery until the broker carries the current campus, assigned-teacher, or self scope; its 11 mutations remain approval-required. The four AI-provider Administration reads expose the provider catalogue, reduced connection health, and cached models; they omit credentials, fingerprints, configuration attribution, wrapping metadata, and provider response bodies. The four routing reads list and inspect reduced route state, expose safe route-configuration choices, or resolve a route fail-closed; no provider credential or internal model-snapshot identifier is returned. Capability-aware resolution derives the canonical module and operation effect from the registered executable capability, rejects contradictory selectors, and preserves the correct module-route fallback when the caller supplies only a capability.
...
Current implementation status: migration 078 and the `cp-ai-providers` platform crate implement tenant-scoped OpenAI, Anthropic, and OpenRouter API-key connections. Administration can list supported providers; create, read, rename, test, rotate, and safely disconnect multiple connections; and read or refresh immutable model snapshots. Every mutation and provider test or refresh outcome writes reduced actor-aware audit evidence. Migration 083 and `cp-agent-runtime` add versioned provider/model route sets for tenant default, task class, module/operation, and exact capability scopes. A route contains one to three ordered targets, binds each target to a ready connection and its current immutable model snapshot, optionally requires confirmed tool support, and replaces or archives under optimistic version checks with actor-aware audit evidence. Resolution uses capability, module/operation, task class, then tenant-default precedence. A matched but unusable route fails closed and never falls through to a broader route. Migrations 085, 086, 091, and 092 add durable Sessions/runs, normalized usage and transactional limits, worker readiness/fencing, and append-only non-executable capability-rejection evidence. Provider execution revalidates connection, credential, approval, model, and tool facts immediately before the sole outbound send boundary.

CCS remains the interaction reference for ChatGPT/Codex, Claude.ai/Claude Code, and Kimi Code subscription connections, but its static CLI OAuth client identities are not a Campus Pilot deployment contract. Campus Pilot may show these providers as unavailable setup choices, but it must not enable their connect actions or create OAuth/device attempts until the deployment has provider-approved client credentials and the matching test, model, refresh, and execution adapter is complete. Successful authentication alone never marks a provider ready.
...
The current generic object-storage route is unauthenticated and configures a public-read bucket. Agent must not use it for prompts, attachments, import sources, capability results, or generated documents. The initial runtime is text and typed-context-reference only. Document Registry provides a separate private tenant-prefixed boundary with bounded signature-checked uploads, internal malware scanning, retention controls, legal holds, durable queued deletion, and human-authorized short-lived downloads. Agent capabilities return authorized registry and legal-hold metadata only; raw bytes, object keys, deletion jobs, and download links never enter provider context. This is not yet a generic Agent attachment service.
...
Current implementation status: `cp-agent::ModuleCoverageRegistry` joins every catalogued module's delivery stage, core/licensed boundary, workspace route, routed product operations, Agent exposure counts, registered executable capabilities, and missing directly exposed handlers. The application catalogue test proves all 23 modules are represented and aligned. Administration exposes 16 executable reads; HR and payroll exposes 13; Fleet exposes seven; Transport exposes five; Facilities exposes six; Activities exposes six; Academics exposes 31; Attendance exposes eight; Learning exposes 21; Student Support exposes three; Communication exposes seven; Timetabling exposes four; SIS exposes 15; Finance exposes 12; Fees exposes 11; Library exposes 14; Hostel exposes 11; Health services exposes eight; Document Registry exposes 11; Internal Audit exposes nine; Procurement exposes ten; and Assets and inventory exposes 13. Every directly exposed operation has a registered typed handler. Activities has 25 routed operations: six scoped typed reads, 18 approval-required mutations, and one human-only SIS/HR reference workflow. It depends on SIS and HR and payroll, returns only the current learner's own record under self scope, and restricts leader mutations to assigned groups even when the same account also has Student access. Facilities has 18 routed operations: six scoped typed reads, 11 approval-required mutations, and one human-only assignment-reference workflow. It depends on HR and payroll, keeps request scope campus-wide or self, keeps work-order scope campus-wide or assigned employee, and re-proves those facts before every Agent read. Internal Audit has 26 routed operations: nine scoped typed reads and 17 approval-required mutations. It depends on Document Registry, keeps plans campus-wide, enforces assigned-auditor engagement scope, and returns only governed evidence-reference snapshots. Hostel has 23 routed operations: 11 scoped typed reads and 12 approval-required mutations. It resolves learner identity through SIS, uses preview fingerprints for allocation and transfer decisions, enforces campus or self occupancy scope, and keeps pastoral results campus-scoped and Sensitive. Health services has 19 routed operations: eight scoped typed reads and 11 approval-required mutations. It resolves patient identity and guardian contacts through SIS and HR without duplicating people, and all capability results are Highly Sensitive and local-only. Library has 34 routed operations: 14 scoped typed reads and 20 approval-required mutations. It resolves borrower identity through SIS and HR, currencies through Finance, and active learner billing accounts through Fees without exposing credentials or creating financial records directly. Attendance has 19 routed operations, eight exposed reads, 11 approval-required mutations, and licensed Academics and SIS dependencies; lesson-session operations additionally require Timetabling. Its handlers re-evaluate campus or exact active teaching-assignment scope, and exact capabilities prove the register, lesson session, exception, or learner belongs to the authenticated tenant before returning sensitive marks. Exception reads remain campus-only, and no capability claims that follow-up communication was sent. Learning has 66 routed operations: 21 scoped typed reads, 39 approval-required mutations, and six human-only private-file and classification-selection workflows, with Academics, SIS, HR and payroll, and Document Registry dependencies. Its spaces, assignments, submissions, quizzes, attempts, feedback, progress, completion, and immutable score-transfer handlers preserve campus, assigned-teacher, and self visibility. Learner and learner-Agent quiz projections never serialize correct-answer flags; governed learner files contribute metadata only. Student Support has 12 routed operations: three Highly Sensitive scoped reads, eight approval-required mutations, and one human-only reference workflow, with SIS as its direct dependency. Its handlers re-evaluate assigned or campus case scope on every call and immediately respect case-team removal. Communication has 15 routed operations, seven exposed reads, eight approval-required mutations, scoped typed handlers, and Academics, SIS, and HR and payroll dependencies only for live audience construction. Gradebook adds 15 Academics operations with licensed SIS and HR and payroll dependencies; its HTTP routes enforce current campus or assigned-teacher role scope, while its six typed Agent reads remain withheld until that role-scope evidence enters the broker handler context. Mark-import reads return only bounded metadata and normalized preview rows, never retained source bytes or unmapped columns. Progress & reporting adds 17 Academics operations with licensed Attendance, SIS, and HR and payroll dependencies; its six typed reads and HTTP routes preserve campus, assigned-teacher, and self visibility, and its typed Agent reads remain withheld until that scope evidence enters the broker context. This registry is diagnostic evidence, not an access grant.

Activities adds 25 routed operations: six scoped catalog, group, and session reads with registered typed handlers, 18 approval-required mutations, and one human-only SIS/HR reference workflow. Each read reloads current authority and `activities.groups` or `activities.sessions` scope. Student results contain only the caller's own membership or participation, while session mutations resolve assigned scope independently so a Student plus Activity Leader account cannot operate an unrelated self-visible group. Completion freezes an immutable roster snapshot.

Document Registry adds 25 routed operations: 11 metadata reads with registered typed handlers, 12 approval-required mutations, and two human-only private-byte workflows. Its handlers reload campus record scope and restricted-record authority, suppress restricted records and derived counts without that authority, and never return raw bytes, object keys, deletion jobs, or download links. Wildcard permission does not substitute for campus record scope. Legal-hold list and record reads are Sensitive; apply and release are approval-required. Restricted authority is rechecked against the stored file inside every mutation and disposition transition.

Transport adds 19 routed operations: five typed reads with registered handlers, 13 approval-required mutations, and one human-only reference workflow. It requires both Fleet and SIS, keeps configuration distinct from day-of-service operation, and re-proves tenant-owned route/run records before returning data. Immutable run snapshots preserve historical learner, stop, vehicle, and driver labels without making those snapshots a second source of current identity.
...
| Academics | subjects, classes, assessment structures, Gradebook, progression, reports | Mark publication and progression changes require preview and approval. |
...
| Attendance | daily and timetable-linked registers, learner marks and history, lesson sessions, exception follow-up, submission, reasoned reopen | Agent reads are live and record-scoped; exceptions are campus-only, while every mutation requires approval and an exact current version. |
| Activities | catalog, groups, leaders, memberships, consent, sessions, participation | Coordinators manage campus records; leaders operate assigned groups; students read only their own date-effective records; Teachers have no seeded access. |
| E-learning | class spaces, units, governed resources, assignments, quizzes, attempts, feedback, progress, completion, reviewed Gradebook score transfers | Agent receives scoped metadata and immutable score-transfer evidence only; learner projections omit answer keys, private file bytes and downloads remain human-only, and transfer mutations require approval. |
| Student support | restricted learner cases, assigned case teams, action history, escalation | All results are Highly Sensitive and local-only; current team membership is rechecked on every call. |
...
| Library | catalogue, copies, memberships, circulation, holds, fines, settings | Catalogue reads are general; borrower and circulation reads enforce current campus/self scope; fine evidence is sensitive; all mutations require approval. |
...
| Transport | routes, stops, rider assignments, service runs, manifests | Configuration is manager-only; operations require complete manifests and typed SIS/Fleet references. |
| Facilities | locations, service requests, work orders, completion, inspections | Teachers and staff request for self only; officers operate assigned work; managers configure, assign, inspect, and close. Employee identity remains HR-owned. |
...
- Agent runtime tables are private persistence details. Only reviewed `cp-agent-runtime` services and the dedicated worker may write them; HTTP handlers, capability adapters, operational modules, and provider adapters cannot issue direct runtime-table SQL. Every worker mutation must match the tenant, run, worker identity, random lease token, and expected monotonic fence version in its write predicate. Database constraints and triggers enforce durable lifecycle shape and evidence integrity, while caller possession of the lease is enforced at this repository boundary because the shared application database role cannot authenticate which Rust call site issued an otherwise valid `UPDATE`.
...
3. **Durable read-only Agent module** — migrations 085, 086, 091, and 092 own durable Sessions, owner membership, append-only messages, runs, queue leases, redacted events, provider-attempt/capability/rejection trails, usage, limits, worker readiness, fencing, cancellation, and recovery. The API, internal worker, licensed `/modules/agent`, history, personal usage, Administration governance, and global drawer are implemented. Keep the catalogue stage `planned` until a licensed provider route completes the release end to end; never expose a queue that only accumulates runs.
