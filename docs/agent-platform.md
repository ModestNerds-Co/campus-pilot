...
Current implementation status: all 275 released server-owned operations have a code-owned Agent exposure classification. One hundred and eleven read operations are exposed, 154 mutations require approval, and ten credential, destructive connection, role, account-deletion, and license-credential operations remain human-only with explicit reasons. This metadata is coverage policy only; it does not make an operation executable until the capability broker and typed capability handler exist.
