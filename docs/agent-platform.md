...
Current implementation status: all 558 released server-owned operations have a code-owned Agent exposure classification. Two hundred and ten read operations are exposed, 311 mutations require approval, 25 credential, destructive connection, private-document-byte, restricted-reference, role, account-deletion, license-credential, and Facilities assignment-reference operations remain human-only with explicit reasons, and 12 operations are prohibited. This metadata is coverage policy only; it does not make an operation executable until the capability broker and typed capability handler exist.
