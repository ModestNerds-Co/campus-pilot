...
Current implementation status: all 265 released server-owned operations have a code-owned Agent exposure classification. One hundred and seven read operations are exposed, 148 mutations require approval, and ten credential, destructive connection, role, account-deletion, and license-credential operations remain human-only with explicit reasons. This metadata is coverage policy only; it does not make an operation executable until the capability broker and typed capability handler exist.
...
Assets and inventory exposes four typed reads for item and store lists/records. Those projections retain the server-generated references and exact quantity-scale metadata while omitting internal creator identifiers. Item/store create, update, and delete are six approval-required mutations and are not registered for direct execution. The current capability boundary contains no stock ledger, movement, balance, custodianship, valuation, or Procurement receipt-allocation operation, so Agent cannot infer or simulate those effects from master-data CRUD.

The current executable registry contains 107 reads: 12 Administration, 13 HR and payroll, seven Fleet, 19 Academics, four Timetabling, 15 SIS, 12 Finance, 11 Fees, ten Procurement, and four Assets and inventory. The four AI-provider Administration reads expose the provider catalogue, reduced connection health, and cached models; they omit credentials, fingerprints, configuration attribution, wrapping metadata, and provider response bodies.
...
| Assets and inventory | item master and stores; stock movement and custodianship planned | The current slice creates no stock balance or Procurement receipt allocation; master-data mutations require approval. |
