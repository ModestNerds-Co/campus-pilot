...
Current implementation status: all 540 released server-owned operations have a code-owned Agent exposure classification. Two hundred and four read operations are exposed, 300 mutations require approval, 24 credential, destructive connection, private-document-byte, restricted-reference, role, account-deletion, and license-credential operations remain human-only with explicit reasons, and 12 operations are prohibited. This metadata is coverage policy only; it does not make an operation executable until the capability broker and typed capability handler exist.
