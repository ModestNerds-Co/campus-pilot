...
Current implementation status: all 615 released server-owned operations have a code-owned Agent exposure classification. Two hundred and twenty-nine read operations are exposed, 348 mutations require approval, 26 credential, destructive connection, private-document-byte, restricted-reference, role, account-deletion, license-credential, and Facilities and Activities assignment-reference operations remain human-only with explicit reasons, and 12 operations are prohibited. This metadata is coverage policy only; it does not make an operation executable until the capability broker and typed capability handler exist.
...
| E-learning | class learning spaces, units, governed resources, publication | Agent receives scoped metadata only; private file bytes and downloads remain human-only. |
