...
Current implementation status: all 509 released server-owned operations have a code-owned Agent exposure classification. One hundred and ninety-six read operations are exposed, 279 mutations require approval, 22 credential, destructive connection, private-document-byte, role, account-deletion, and license-credential operations remain human-only with explicit reasons, and 12 operations are prohibited. This metadata is coverage policy only; it does not make an operation executable until the capability broker and typed capability handler exist.
