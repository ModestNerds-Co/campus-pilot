...
Document Registry adds 21 routed operations: nine metadata reads with registered typed handlers, ten approval-required mutations, and two human-only private-byte workflows. Its handlers reload campus record scope and restricted-record authority, and never return raw bytes, object keys, or download links.
