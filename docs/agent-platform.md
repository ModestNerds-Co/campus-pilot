...
Current implementation status: `cp-agent::ModuleCoverageRegistry` joins every catalogued module's delivery stage, core/licensed boundary, workspace route, routed product operations, Agent exposure counts, registered executable capabilities, and missing directly exposed handlers. The application catalogue test proves all 18 modules are represented and aligned. Administration exposes 16 executable reads; HR and payroll exposes 13; Fleet exposes seven; Academics exposes 28; Attendance exposes three; Timetabling exposes four; SIS exposes 15; Finance exposes 12; Fees exposes 11; Procurement exposes ten; and Assets and inventory exposes 13. Every directly exposed operation has a registered typed handler. Attendance has eight routed operations, three exposed reads, five approval-required mutations, and licensed Academics and SIS dependencies. Its exact record capability proves the register belongs to the authenticated tenant before returning sensitive learner marks. Gradebook adds nine Academics operations with licensed SIS and HR and payroll dependencies; its HTTP routes enforce current campus or assigned-teacher role scope, while its three typed Agent reads remain withheld until that role-scope evidence enters the broker handler context. Progress & reporting adds 17 Academics operations with licensed Attendance, SIS, and HR and payroll dependencies; its six typed reads and HTTP routes preserve campus, assigned-teacher, and self visibility, and its typed Agent reads remain withheld until that scope evidence enters the broker context. This registry is diagnostic evidence, not an access grant.
