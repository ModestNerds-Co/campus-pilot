/** Assignment brief, rubric, participation, and review workspace. */
...
import type { LearningAssignment, LearningRubricCriterion, LearningSpace, LearningSubmissionMethod } from "./types";
...
export function LearningAssignmentWorkspace({ assignmentId, onSearchChange, search, spaceId }: {
  ...
  return <div className="space-y-6">
    ...
    <section className="border border-[var(--border)] bg-[var(--surface)] p-5 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex flex-wrap items-center gap-2"><LearningStatusBadge status={assignment.status} /><span className="text-xs font-medium text-[var(--text-muted)]">{unit ? `Unit ${unit.position} · ${unit.title}` : "Unit unavailable"}</span></div><p className="mt-3 text-sm text-[var(--text-muted)]">Due {formatLearningDateTime(assignment.due_at)} · Maximum {formatHundredths(assignment.max_score_hundredths)} · {submissionMethodLabel(assignment.submission_method)}</p></div>{canTeach ? <div className="text-right"><p className="font-tabular text-lg font-semibold text-[var(--text-strong)]">{assignment.submission_count} / {assignment.recipient_count || "—"}</p><p className="text-xs text-[var(--text-muted)]">started submissions</p></div> : null}</div>
...
function submissionMethodLabel(method: LearningSubmissionMethod) { return method === "text" ? "Text response" : method === "file" ? "File response" : "Text or file response"; }
