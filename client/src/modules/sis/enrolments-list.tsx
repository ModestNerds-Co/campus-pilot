...
export function EnrolmentsList() {
  ...
  usePageChrome("Enrolments", <Button onClick={() => setDrawerRecord(null)}><Plus className="size-4" />Add enrolment</Button>);
  ...
  return <div className="space-y-6">
    ...
    <TableWrap>{loading ? <TableLoading columns={6} label="Loading enrolments…" /> : error ? <TableError description={error} onRetry={() => void load()} /> : records.length === 0 ? <TableEmpty description={filtered ? "Change the current filters." : "Add the first class enrolment."} icon={<GraduationCap />} title={filtered ? "No enrolments match these filters" : "No enrolments yet"} /> : <TableScroll><Table><THead><tr><TH>Learner</TH><TH>Academic year</TH><TH>Class</TH><TH>Dates</TH><TH>Status</TH><TH className="text-right">Actions</TH></tr></THead><TBody>{records.map((record) => <TR key={record.id}><TD><div className="font-medium text-[var(--text-strong)]">{record.learner_name}</div><div className="font-tabular text-xs text-[var(--text-muted)]">{record.learner_number}</div></TD><TD className="text-[var(--text-muted)]">{record.academic_year_name}</TD><TD><div className="font-medium text-[var(--text-strong)]">{record.class_group_name}</div>{record.application_number ? <div className="font-tabular text-xs text-[var(--text-muted)]">{record.application_number}</div> : null}</TD><TD className="text-[var(--text-muted)]">{formatDate(record.starts_on)}{record.ends_on ? ` – ${formatDate(record.ends_on)}` : ""}</TD><TD><Badge tone={record.status === "active" ? "success" : record.status === "withdrawn" ? "danger" : "neutral"}>{record.status}</Badge></TD><TD className="text-right"><div className="relative inline-flex"><button aria-label="Enrolment actions" className="inline-flex size-8 items-center justify-center rounded-[var(--radius-md)] hover:bg-[var(--surface-muted)]" onClick={() => setMenuId(menuId === record.id ? null : record.id)} type="button"><MoreVertical className="size-4" /></button>{menuId === record.id ? <div className="absolute right-0 top-9 z-10 w-40 rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)] py-1 shadow-[var(--shadow-popover)]"><button className="flex w-full items-center gap-2 px-4 py-2 text-sm hover:bg-[var(--surface-muted)]" onClick={() => { setDrawerRecord(record); setMenuId(null); }} type="button"><Edit className="size-4" />Edit</button></div> : null}</div></TD></TR>)}</TBody></Table></TableScroll>}</TableWrap>
...
function EnrolmentDrawer({ onClose, onSaved, open, record }: { onClose: () => void; onSaved: () => void; open: boolean; record: Enrolment | null }) {
  ...
  useEffect(() => {
    if (!open) return;
    setLearnerId(record?.learner_id ?? ""); setAcademicYearId(record?.academic_year_id ?? ""); setClassId(record?.class_group_id ?? ""); setApplicationId(record?.source_application_id ?? ""); setStartsOn(record?.starts_on ?? ""); setEndsOn(record?.ends_on ?? ""); setStatus(record?.status ?? "active");
    setLoading(true);
  ...
  const availableApplications = applications.filter((item) => (!learnerId || item.learner_id === learnerId) && (!academicYearId || item.academic_year_id === academicYearId));
  ...
  return <DialogShell onClose={onClose} open={open}><DialogHeader onClose={onClose} title={`${record ? "Edit" : "Add"} enrolment`} /><form onSubmit={submit}><DialogBody className="space-y-5">
    <div><Label>Learner</Label><Select className="mt-1.5" data-autofocus="true" disabled={loading || record !== null} onChange={(event) => { setLearnerId(event.target.value); setApplicationId(""); }} required value={learnerId}><option value="">Choose a learner</option>{learners.map((learner) => <option key={learner.id} value={learner.id}>{learner.display_name} · {learner.learner_number}</option>)}</Select></div>
    <div><Label>Academic year</Label><Select className="mt-1.5" disabled={loading} onChange={(event) => { setAcademicYearId(event.target.value); setClassId(""); setApplicationId(""); }} required value={academicYearId}><option value="">Choose an academic year</option>{years.map((year) => <option key={year.id} value={year.id}>{year.name} · {year.status}</option>)}</Select></div>
    <div><Label>Class</Label><Select className="mt-1.5" disabled={loading || !academicYearId} onChange={(event) => setClassId(event.target.value)} required value={classId}><option value="">Choose a class</option>{availableClasses.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select></div>
    <div><Label>Accepted application</Label><Select className="mt-1.5" disabled={loading || !learnerId || !academicYearId} onChange={(event) => setApplicationId(event.target.value)} value={applicationId}><option value="">No source application</option>{availableApplications.map((application) => <option key={application.id} value={application.id}>{application.application_number}</option>)}</Select></div>
    <div className="grid gap-4 sm:grid-cols-2"><div><Label>Start date</Label><Input className="mt-1.5" onChange={(event) => setStartsOn(event.target.value)} required type="date" value={startsOn} /></div><div><Label>End date</Label><Input className="mt-1.5" min={startsOn || undefined} onChange={(event) => setEndsOn(event.target.value)} type="date" value={endsOn} /></div></div>
    <div><Label>Status</Label><Select className="mt-1.5" onChange={(event) => setStatus(event.target.value as EnrolmentStatus)} value={status}><option value="active">Active</option><option value="completed">Completed</option><option value="withdrawn">Withdrawn</option></Select></div>
