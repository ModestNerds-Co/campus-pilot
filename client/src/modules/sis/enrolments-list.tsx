...
function EnrolmentDrawer({ onClose, onSaved, open, record }: { onClose: () => void; onSaved: () => void; open: boolean; record: Enrolment | null }) {
  ...
  return <DialogShell onClose={onClose} open={open}><DialogHeader onClose={onClose} title={`${record ? "Edit" : "Add"} enrolment`} /><form onSubmit={submit}><DialogBody className="space-y-5">
    ...
    <div><Label>Class</Label><Select className="mt-1.5" disabled={loading || !academicYearId} onChange={(event) => { setClassId(event.target.value); setApplicationId(""); }} required value={classId}><option value="">Choose a class</option>{availableClasses.map((item) => <option key={item.id} value={item.id}>{item.name}{item.grade_level ? ` · ${item.grade_level}` : ""}</option>)}</Select></div>
    <div><Label>Accepted application</Label><Select className="mt-1.5" disabled={loading || !learnerId || !academicYearId || !classId} onChange={(event) => setApplicationId(event.target.value)} value={applicationId}><option value="">No source application</option>{availableApplications.map((application) => <option key={application.id} value={application.id}>{application.application_number} · {application.target_grade_level_name}</option>)}</Select></div>
