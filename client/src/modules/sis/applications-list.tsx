...
function ApplicationDrawer({ onClose, onSaved, open, record }: { onClose: () => void; onSaved: () => void; open: boolean; record: Application | null }) {
  ...
  return <DialogShell onClose={onClose} open={open}><DialogHeader onClose={onClose} title={`${record ? "Edit" : "Add"} application`} /><form onSubmit={submit}><DialogBody className="space-y-5">
    ...
    <div><Label>Academic year</Label><Select className="mt-1.5" disabled={loading || choicesLocked} onChange={(event) => setAcademicYearId(event.target.value)} required value={academicYearId}><option value="">Choose an academic year</option>{years.map((year) => <option key={year.id} value={year.id}>{year.name} · {year.status}</option>)}</Select></div>
    <div><Label>Target grade</Label><Select className="mt-1.5" disabled={loading || (choicesLocked && record?.target_grade_level_id !== null)} onChange={(event) => setGradeLevelId(event.target.value)} required value={gradeLevelId}><option value="">Choose a grade level</option>{gradeLevels.filter((grade) => grade.status === "active" || grade.id === record?.target_grade_level_id).map((grade) => <option key={grade.id} value={grade.id}>{grade.name} · {grade.code}</option>)}</Select></div>
