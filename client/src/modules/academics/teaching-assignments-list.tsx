...
export function TeachingAssignmentsList() {
  ...
  usePageChrome("Teaching assignments", <Button onClick={() => setDrawerAssignment(null)}><Plus className="size-4" />Add assignment</Button>);
  ...
  return <div className="space-y-6">
    ...
    <TableWrap>{loading ? <TableLoading columns={6} label="Loading teaching assignments…" /> : error ? <TableError description={error} onRetry={() => void load()} /> : assignments.length === 0 ? <TableEmpty description={filtered ? "Change the current filters." : "Add the first class, subject, and teacher assignment."} icon={<ClipboardList />} title={filtered ? "No assignments match these filters" : "No teaching assignments yet"} /> : <TableScroll><Table><THead><tr><TH>Class</TH><TH>Subject</TH><TH>Teacher</TH><TH>Academic year</TH><TH>Teaching load</TH><TH>Status</TH><TH className="text-right">Actions</TH></tr></THead><TBody>{assignments.map((assignment) => <TR key={assignment.id}><TD className="font-medium text-[var(--text-strong)]">{assignment.class_group_name}</TD><TD className="text-[var(--text-strong)]">{assignment.subject_name}</TD><TD><div className="text-[var(--text-strong)]">{assignment.teacher_name}</div><div className="text-xs text-[var(--text-muted)]">Employee-backed</div></TD><TD className="text-[var(--text-muted)]">{assignment.academic_year_name}</TD><TD className="font-tabular text-[var(--text-muted)]">{assignment.periods_per_cycle} periods</TD><TD><Badge tone={assignment.status === "active" ? "success" : "neutral"}>{assignment.status}</Badge></TD><TD className="text-right"><div className="relative inline-flex"><button aria-label="Assignment actions" className="inline-flex size-8 items-center justify-center rounded-[var(--radius-md)] hover:bg-[var(--surface-muted)]" onClick={() => setMenuId(menuId === assignment.id ? null : assignment.id)} type="button"><MoreVertical className="size-4" /></button>{menuId === assignment.id ? <div className="absolute right-0 top-9 z-10 w-40 rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)] py-1 shadow-[var(--shadow-popover)]"><button className="flex w-full items-center gap-2 px-4 py-2 text-sm hover:bg-[var(--surface-muted)]" onClick={() => { setDrawerAssignment(assignment); setMenuId(null); }} type="button"><Edit className="size-4" />Edit</button><button className="flex w-full items-center gap-2 px-4 py-2 text-sm text-[var(--tone-danger)] hover:bg-[var(--tone-danger-bg)]" onClick={() => { setDeleteAssignment(assignment); setMenuId(null); }} type="button"><Trash2 className="size-4" />Remove</button></div> : null}</div></TD></TR>)}</TBody></Table></TableScroll>}</TableWrap>
    <AssignmentDrawer assignment={drawerAssignment ?? null} onClose={() => setDrawerAssignment(undefined)} onSaved={() => { setDrawerAssignment(undefined); void load(); }} open={drawerAssignment !== undefined} years={years} />
    <ConfirmDrawer confirmLabel="Remove assignment" description={`Remove ${deleteAssignment ? `${deleteAssignment.teacher_name}'s ${deleteAssignment.subject_name} assignment for ${deleteAssignment.class_group_name}` : "this teaching assignment"}?`} isPending={deleting} onClose={() => setDeleteAssignment(null)} onConfirm={() => void remove()} open={deleteAssignment !== null} title="Remove teaching assignment?" />
  </div>;
}
