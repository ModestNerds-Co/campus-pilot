...
export function TeachersList() {
  ...
  usePageChrome("Teachers", canCreate ? <Button onClick={() => setDrawerTeacher(null)}><Plus className="size-4" />Add teacher</Button> : null);
  ...
  return <div className="space-y-6">
    ...
    <TableWrap>{loading ? <TableLoading columns={5} label="Loading teachers…" /> : error ? <TableError description={error} onRetry={() => void load()} /> : teachers.length === 0 ? <TableEmpty description={filtered ? "Change the current filters." : "No teacher profiles have been created."} icon={<UserRoundCheck />} title={filtered ? "No teachers match these filters" : "No teachers yet"} /> : <TableScroll><Table><THead><tr><TH>Teacher</TH><TH>Employee</TH><TH>Contact</TH><TH>Status</TH><TH className="text-right">Actions</TH></tr></THead><TBody>{teachers.map((teacher) => <TR key={teacher.id}><TD><div className="font-medium text-[var(--text-strong)]">{teacher.display_name}</div><div className="text-xs text-[var(--text-muted)]">Academics profile</div></TD><TD><div className="font-tabular text-[var(--text-strong)]">{teacher.employee_number}</div><div className="text-xs text-[var(--text-muted)]">{teacher.employment_status}</div></TD><TD className="text-[var(--text-muted)]">{teacher.work_email || teacher.phone || "—"}</TD><TD><Badge tone={teacher.status === "active" ? "success" : "neutral"}>{teacher.status}</Badge></TD><TD className="text-right">{canEdit || canDelete ? <div className="relative inline-flex"><button aria-label="Teacher actions" className="inline-flex size-8 items-center justify-center rounded-[var(--radius-md)] hover:bg-[var(--surface-muted)]" onClick={() => setMenuId(menuId === teacher.id ? null : teacher.id)} type="button"><MoreVertical className="size-4" /></button>{menuId === teacher.id ? <div className="absolute right-0 top-9 z-10 w-48 rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)] py-1 shadow-[var(--shadow-popover)]">{canEdit ? <button className="flex w-full items-center gap-2 px-4 py-2 text-sm hover:bg-[var(--surface-muted)]" onClick={() => { setDrawerTeacher(teacher); setMenuId(null); }} type="button"><Edit className="size-4" />Change status</button> : null}{canDelete ? <button className="flex w-full items-center gap-2 px-4 py-2 text-sm text-[var(--tone-danger)] hover:bg-[var(--tone-danger-bg)]" onClick={() => { setDeleteTeacher(teacher); setMenuId(null); }} type="button"><Trash2 className="size-4" />Remove profile</button> : null}</div> : null}</div> : <span className="text-[var(--text-subtle)]">—</span>}</TD></TR>)}</TBody></Table></TableScroll>}</TableWrap>
    <TeacherDrawer onClose={() => setDrawerTeacher(undefined)} onSaved={() => { setDrawerTeacher(undefined); void load(); }} open={(canCreate || canEdit) && drawerTeacher !== undefined} teacher={drawerTeacher ?? null} />
    <ConfirmDrawer confirmLabel="Remove teacher profile" description={`Remove ${deleteTeacher?.display_name || "this teacher"} from Academics? Their HR employee record will not be removed. Teaching assignments must be removed first.`} isPending={deleting} onClose={() => setDeleteTeacher(null)} onConfirm={() => void remove()} open={canDelete && deleteTeacher !== null} title="Remove teacher profile?" />
  </div>;
}
