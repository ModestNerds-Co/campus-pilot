...
import type { AcademicYear, AcademicYearStatus, ClassGroup, DirectoryStatus, Subject } from "./types";
...
export function AcademicDirectoryList({ kind }: { kind: DirectoryKind }) {
  ...
  }, [kind, label.plural, page, status, submittedSearch, yearFilter]);
  ...
  const filtered = submittedSearch || status !== "all" || (kind === "class" && yearFilter !== "all");
  ...
  return (
    <div className="space-y-6">
      ...
      <DirectoryDrawer kind={kind} onClose={() => setDrawerRecord(undefined)} onSaved={() => { setDrawerRecord(undefined); void load(); }} open={drawerRecord !== undefined} record={drawerRecord ?? null} years={years} />
...
function DirectoryDrawer({ kind, onClose, onSaved, open, record, years }: { kind: DirectoryKind; onClose: () => void; onSaved: () => void; open: boolean; record: DirectoryRecord | null; years: AcademicYear[] }) {
  const label = labels[kind];
  ...
  useEffect(() => {
    ...
    setGradeLevel(record && "grade_level" in record ? record.grade_level ?? "" : "");
  ...
  return <DialogShell onClose={onClose} open={open}><DialogHeader onClose={onClose} title={`${record ? "Edit" : "Add"} ${label.singular}`} /><form onSubmit={submit}><DialogBody className="space-y-5">
    ...
    {kind === "class" ? <div><Label>Grade level</Label><Input className="mt-1.5" maxLength={80} onChange={(event) => setGradeLevel(event.target.value)} placeholder="e.g. Form 1" value={gradeLevel} /></div> : null}
