/** Staged CSV/XLSX imports into learner billing accounts. */
...
import type {
  FeesImportDateFormat, FeesImportMapping, FeesImportPreview,
  FeesImportPreviewRow, FeesImportRecord,
} from "./types";

const MAPPING_FIELDS = [
  ["learner_number", "Learner number", true],
  ["account_number", "Existing account number", false],
  ["opened_on", "Opened on", true],
  ["status", "Status", false],
  ["closed_on", "Closed on", false],
] as const;

const HEADER_ALIASES: Record<string, string[]> = {
  learner_number: ["learnernumber", "studentnumber", "admissionnumber", "studentid", "learnerid"],
  account_number: ["accountnumber", "billingaccount", "billingaccountnumber", "accountno"],
  opened_on: ["openedon", "openeddate", "startdate", "createddate"],
  status: ["status", "accountstatus"],
  closed_on: ["closedon", "closeddate", "enddate"],
};

export function FeesImportsWorkspace() {
  const [records, setRecords] = useState<FeesImportRecord[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [page, setPage] = useState(1);
  const [totalPages, setTotalPages] = useState(1);
  const [uploadOpen, setUploadOpen] = useState(false);
  const [selected, setSelected] = useState<FeesImportRecord | null>(null);

  const load = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const response = await feesService.listImports({ page, per_page: 20 });
      if (!response.success || !response.data) throw new Error(responseMessage(response, "Imports could not be loaded"));
      setRecords(response.data.imports);
      setTotalPages(response.pagination?.total_pages ?? 1);
    } catch (loadError) {
      setError(networkMessage(loadError, "Imports could not be loaded"));
    } finally {
      setLoading(false);
    }
  }, [page]);
  ...
  usePageChrome("Billing account imports", action);

  return <div className="space-y-6">
    <p className="text-sm text-[var(--text-muted)]">Upload billing accounts, map the columns, review each row, then commit the ready rows.</p>
    <TableControlsBar>
      <div />
    ...
    <TableWrap>
      {loading ? <TableLoading columns={5} label="Loading imports…" /> : error ? <TableError description={error} onRetry={() => void load()} /> : records.length === 0 ? <TableEmpty description="Upload a billing-account file." icon={<FileSpreadsheet />} title="No imports yet" /> : <TableScroll><Table>
        <THead><tr><TH>File</TH><TH>Rows</TH><TH>Preview</TH><TH>Status</TH><TH className="text-right">Action</TH></tr></THead>
...
function UploadImportDrawer({ onClose, onUploaded, open }: { onClose: () => void; onUploaded: (record: FeesImportRecord) => void; open: boolean }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setUploading(true);
    try {
      const response = await feesService.uploadImport(file);
      if (!response.success || !response.data) { toast.error(responseMessage(response, "The import could not be uploaded")); return; }
  ...
  return <DialogShell onClose={() => !uploading && onClose()} open={open}>
    <DialogHeader onClose={() => !uploading && onClose()} title="New billing account import" />
    <form onSubmit={submit}>
      <DialogBody className="space-y-6">
        <div><Label htmlFor="fees-import-file">CSV or XLSX file</Label><Input accept=".csv,.xlsx,text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" className="mt-1.5 h-auto py-2" data-autofocus="true" id="fees-import-file" onChange={(event) => setFile(event.target.files?.[0] ?? null)} required type="file" /><p className="mt-2 text-xs text-[var(--text-muted)]">Maximum 5 MB and 5,000 data rows. The first row must contain unique column names.</p></div>
...
function ImportReviewDrawer({ onChanged, onClose, record }: { onChanged: () => void; onClose: () => void; record: FeesImportRecord | null }) {
  const [current, setCurrent] = useState<FeesImportRecord | null>(record);
  const [preview, setPreview] = useState<FeesImportPreview | null>(null);
  const [mapping, setMapping] = useState<Record<string, string>>({});
  const [dateFormat, setDateFormat] = useState<FeesImportDateFormat>("yyyy_mm_dd");
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void feesService.getImportPreview(record.id, { page: 1, per_page: 100 }).then((response) => {
      if (response.success && response.data) {
        setPreview(response.data);
        setMapping(response.data.mapping.columns);
        setDateFormat(response.data.mapping.date_format ?? "yyyy_mm_dd");
      } else toast.error(responseMessage(response, "The import preview could not be loaded"));
  ...
  const validate = async () => {
    ...
    setSaving(true);
    try {
      const payload: FeesImportMapping = { columns: Object.fromEntries(Object.entries(mapping).filter(([, header]) => header)), date_format: dateFormat };
      const response = await feesService.createImportPreview(current.id, payload);
      if (!response.success || !response.data) { toast.error(responseMessage(response, "The mapping could not be validated")); return; }
      setPreview(response.data);
      setMode("preview");
  ...
  const commit = async () => {
    ...
    setSaving(true);
    try {
      const response = await feesService.commitImport(current.id, preview.id);
      if (!response.success || !response.data) { toast.error(responseMessage(response, "The import could not be committed")); return; }
      const refreshed = await feesService.getImport(current.id);
      if (refreshed.success && refreshed.data) setCurrent(refreshed.data);
      toast.success(`${response.data.created_rows} billing accounts created`);
      onChanged();
  ...
  return <DialogShell onClose={() => !saving && onClose()} open={record !== null} panelClassName="sm:max-w-[780px]">
    <DialogHeader onClose={() => !saving && onClose()} title="Billing account import" />
    <DialogBody className="space-y-6">
      {!current ? null : <>
        ...
        {loading ? <div className="flex items-center gap-2 py-8 text-sm text-[var(--text-muted)]"><Loader2 className="size-4 animate-spin" />Loading preview…</div> : mode === "mapping" ? <div className="space-y-5">
          <div><h3 className="text-sm font-semibold text-[var(--text-strong)]">Column mapping</h3><p className="mt-1 text-xs leading-5 text-[var(--text-muted)]">Learner numbers must match existing SIS records. Leave account number unmapped to generate Campus Pilot numbers.</p></div>
          <div className="space-y-4">{MAPPING_FIELDS.map(([key, label, required]) => <div className="grid gap-2 sm:grid-cols-[190px_minmax(0,1fr)] sm:items-center" key={key}>
            <Label htmlFor={`fees-mapping-${key}`}>{label}{required ? <span className="text-[var(--tone-danger)]"> *</span> : null}</Label>
            <Select id={`fees-mapping-${key}`} onChange={(event) => setMapping((value) => ({ ...value, [key]: event.target.value }))} value={mapping[key] ?? ""}>
          ...
          <div><Label htmlFor="fees-import-date-format">Date format</Label><Select className="mt-1.5" id="fees-import-date-format" onChange={(event) => setDateFormat(event.target.value as FeesImportDateFormat)} value={dateFormat}><option value="yyyy_mm_dd">YYYY-MM-DD</option><option value="dd_mm_yyyy">DD/MM/YYYY</option><option value="mm_dd_yyyy">MM/DD/YYYY</option></Select></div>
...
function PreviewContent({ committed, preview }: { committed: boolean; preview: FeesImportPreview }) {
...
function PreviewRow({ row }: { row: FeesImportPreviewRow }) {
  const learnerName = stringValue(row.canonical_data.learner_name);
  const learnerNumber = stringValue(row.canonical_data.learner_number);
  const accountNumber = stringValue(row.canonical_data.account_number);
  return <tr><td className="px-4 py-3 align-top font-tabular text-[var(--text-muted)]">{row.row_number}</td><td className="px-4 py-3 align-top"><p className="font-medium text-[var(--text-strong)]">{learnerName ?? "Billing account row"}</p><p className="mt-1 text-xs text-[var(--text-muted)]">{learnerNumber ?? `Source row ${row.row_number}`}{accountNumber ? ` · ${accountNumber}` : ""}</p></td><td className="px-4 py-3 align-top"><Badge tone={row.outcome === "ready" ? "success" : row.outcome === "duplicate" ? "warning" : "danger"}>{displayStatus(row.outcome)}</Badge>{row.issues.length > 0 ? <ul className="mt-2 space-y-1 text-xs leading-4 text-[var(--text-muted)]">{row.issues.map((issue) => <li key={issue}>{issue}</li>)}</ul> : null}</td></tr>;
}
...
function PreviewTotals({ record }: { record: FeesImportRecord }) { return record.ready_rows === null ? <span className="text-xs text-[var(--text-muted)]">Not validated</span> : <span className="text-xs text-[var(--text-muted)]"><span className="font-tabular font-medium text-[var(--text-strong)]">{record.ready_rows}</span> ready · {record.invalid_rows ?? 0} invalid · {record.duplicate_rows ?? 0} duplicate</span>; }
...
function statusTone(status: FeesImportRecord["status"]): "neutral" | "warning" | "success" { return status === "committed" ? "success" : status === "preview_ready" ? "warning" : "neutral"; }
